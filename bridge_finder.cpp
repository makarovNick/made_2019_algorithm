...
// Ввод	bridges.in
// Вывод	bridges.out
// Ребро неориентированного графа называется мостом, если удаление этого ребра из графа увеличивает
// число компонент связности.
// Дан неориентированный граф, требуется найти в нем все мосты.
// Формат ввода
// Первая строка входного файла содержит два целых числа n и m — количество вершин и ребер графа
// соответственно (1 ≤ n ≤ 20000, 1 ≤ m ≤ 200000).
// Следующие m строк содержат описание ребер по одному на строке.
// Ребро номер i описывается двумя натуральными числами bi, ei — номерами концов ребра (1 ≤ bi, ei ≤
// n).
// Формат вывода
// Первая строка выходного файла должна содержать одно натуральное число b — количество мостов в
// заданном графе.
// На следующей строке выведите b целых чисел — номера ребер, которые являются мостами, в
...
//	its descendants in the DFS traversal tree has a back-edge to vertex v or any of its
//ancestors.
...
//	We introduce an array low which will let us check the fact for each vertex v.low[v] is the
//minimum of tin[v],
//	the entry times tin[p] for each node p that is connected to node v via a back - edge(v, p)
//	and the values of low[to] for each vertex to which is a direct descendant of v in the DFS
//tree :
//
//				       ⎧ tin[v]    , otherwise
...
//	If low[to] = tin[v], the back edge comes directly to v, otherwise it comes to one of the
//ancestors of v.
//
...
struct Graph
{
    ...
    {
        edges[from].push_back(std::make_pair(to, index));
        edges[to].push_back(std::make_pair(from, index));
    }
...
{
public:
    BridgeFinder(const Graph& _graph_with_indexes);
...
BridgeFinder::BridgeFinder(const Graph& _graph_with_indexes)
...
void BridgeFinder::DFS(int v, int p)
{
	...
	auto edges = graph.getEdgesFrom(v);
    ...
    for (auto& i : edges)
    {
        if (i.first != p)
        {
            ...
            else
            {
                ...
                {
                    if (std::count_if(edges.begin(), edges.end(),
                            [&](auto& p)
                            {
